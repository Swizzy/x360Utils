    ...
    internal class SMCConfig {
        ...
        public void VerifySMCConfigChecksum(ref byte[] smcconfigdata) {
            ...
            if(checkSum == calculatedCheckSum)
                return;
            Debug.SendDebug("ERROR: SMC_Config Checksums don't match! Expected: {0} Calculated: {1}", checkSum, calculatedCheckSum);
        ...
        public string GetVideoRegion(ref byte[] smcconfigdata) {
            return Translators.TranslateVideoRegion(string.Format("0x{0:X}{1:X}", smcconfigdata[0x22A], smcconfigdata[0x22B]));
        }

        public string GetGameRegion(ref byte[] smcconfigdata, bool includebytes = false) {
            return Translators.TranslateGameRegion(string.Format("0x{0:X}{1:X}", smcconfigdata[0x22C], smcconfigdata[0x22D]), includebytes);
        }
        ...
        {
            return string.Format("{0:X2}:{1:X2}:{2:X2}:{3:X2}:{4:X2}:{5:X2}", smcconfigdata[0x220], smcconfigdata[0x221], smcconfigdata[0x222], smcconfigdata[0x223], smcconfigdata[0x224], smcconfigdata[0x225]);
        }
    }
}